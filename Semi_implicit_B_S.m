% semi_plicit difference scheme for a European-type option in Black-Scholes
% equations
%--------------------------------------------------------------------------
% INPUTS:
%
...
% analysis
%--------------------------------------------------------------------------
function [S,V,A] = Semi_implicit_B_S(K,S_max,r,T,sigma,ds,dt,option_type)
...
switch option_type
    ...
    case 'PUT'
        ...
        % Impose boundary condition under the European call: 
        % V(0,t) = 0 & V(S,t) = K*exp(-rt) as S goes to infinity
...
a = - 0.5 * dt * sigma^2 * veci.^2;
b = 1 + r * dt + r * veci * dt + sigma^2 * dt * veci.^2;
c = - r * veci * dt - 0.5 * sigma^2 * dt * veci.^2;
